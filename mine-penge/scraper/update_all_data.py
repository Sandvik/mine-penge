...
class DataUpdater:
    ...
    def run_all_scrapers(self):
        ...
        for script in self.scraper_scripts:
            if self.run_scraper(script):
                ...
                logger.warning(f"⚠️ {script} fejlede - fortsætter med næste")
    ...
    def check_for_duplicates(self):
        ...
        try:
                ...
                try:
                    with open(filepath, 'r', encoding='utf-8') as f:
                        ...
                        if 'blog_posts' in data:
                            all_articles.extend(data['blog_posts'])
                except Exception as e:
                    logger.warning(f"⚠️ Kunne ikke læse {filename}: {e}")
            ...
            if duplicates > 0:
                logger.warning(f"⚠️ Fundet {duplicates} dubletter i data")
            else:
    ...
    def generate_summary_report(self):
        ...
        for filename in json_files:
            ...
            try:
                with open(filepath, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                    article_count = len(data.get('blog_posts', []))
    ...
    def run_full_update(self):
        ...
        if not self.run_low_value_cleaning():
            print("⚠️ Lavværdi artikel rensning fejlede - fortsætter alligevel")
        else:
            print("✅ Lavværdi artikel rensning gennemført!")
        
        # Trin 7: Kør build_articles.py igen for at samle alle artikler
        print("\n🔨 Kører build_articles.py igen for at samle alle artikler...")
        try:
            build_script = os.path.join(os.path.dirname(__file__), "build_articles.py")
            result = subprocess.run([sys.executable, build_script], check=True)
            print("✅ Samlet articles.json er nu opdateret!")
